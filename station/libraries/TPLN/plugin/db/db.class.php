...
// DataBase ******************************************************************************************************************* @author H2LSOFT */
	 ...
	 * @since 2.8
	 * @author H2LSOFT */
	public function dbInsert($table, $arr, $exlude_fields=array(), $return_last_id=false)
	{
		// protection
	...
	* @author H2LSOFT */
	...
	protected function applyPrivateVar()
	{
		...
		if($this->blocExists('previous'))
		{
			...
			if($this->PageNumber <= $this->PageCount /*&& $this->PageCount != 1 && $this->PageNumber != 1* @author H2LSOFT */)
			{
		...
		{
		* @author H2LSOFT */	$next_pg = $this->PageNumber + 1;
	...
	public function showRecords($query, $nb_result_per_page = 0, $func_data = '')
	{
		...
		{			
			...
			$this->setQueryLimit(); // request which conyains the limits
